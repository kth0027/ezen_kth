...
function signupcheck() {
	...
	if (!pwj.test(password) || !pwj.test(passwordconfirm)) {
		...
		document.getElementById("pwresult").style.color = "blue";
	}
	...
	if (!namej.test(name)) {
		...
		document.getElementById("nameresult").style.color = "blue";
	}
	//생년월일
	if (birth == "") {
		...
		document.getElementById("birthresult").style.color = "blue";
	}
	// 성별
	if (!sex1 && !sex2) {  // 둘다 false 이면 // 둘다 체크를 안했으면
		...
		document.getElementById("sexresult").style.color = "blue";
	}
	// 전화번호 
	if (!phonej.test(phone)) {
		...
		document.getElementById("phoneresult").style.color = "blue";
	}
}
