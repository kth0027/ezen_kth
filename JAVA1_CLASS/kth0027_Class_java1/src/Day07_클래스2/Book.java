...
public class Book {
	...
	public boolean book_add() {
		...
		String b_ISBN = BookApplication.scanner.next();
		// 도서코드 중복체크
		...
		String b_name = BookApplication.scanner.next();
	...
	public void book_list() {
		...
		for (int i = 0; i < BookApplication.books.length; i++) {
			...
			System.out
					.println(book.getB_ISBN() + "\t" + book.getB_name() + "\t" + book.getB_rental() + "\t" + book.m_id);
		}
		return;
	}
	...
	public boolean book_rental(String loginid) {
		...
		if (check) {
			...
			for (int i = 0; i < BookApplication.books.length; i++) {
				if (BookApplication.books[i] != null && BookApplication.books[i].getB_ISBN().equals(b_ISBN)) { // 동일한
																												// 도서가
																												// 존재하면
					// 대여여부 확인
					if (BookApplication.books[i].getB_rental()) {
						...
						System.out.println(" [[ 알림 ]] : 현재 대여중인 도서 입니다. 대여불가!!!");
						return false;
					}
				}
			}
