...
<body>
	...
	<div class="container">
		...
		<table class="table">
			...
			<tr>
				...
				<td class="row no-gutters">
					<button class="btn btn-outline-secondary" onclick="pchange2( <%=i %> , 'p' , <%=product.getP_stock() %> , <%=product.getP_price() %>)">+</button>
					<div class="col-md-3">
						<input class="form-control" type="text" id="pcount<%=i %>" value=<%=cart.getP_count() %> name="p_count" onchange="pchange2(<%=i %> ,'s' , <%=product.getP_stock()%>  , <%=product.getP_price() %>  )">
					</div>
		...
		<div class="text-center totalbox">
			<p>
				총 상품 가격 <span class="carttotal"><%= decimalFormat.format(totalprice) %></span> + 총 배송비 <span class="carttotal">3,000</span> = 총 주문금액 : <span class="carttotal"><%= decimalFormat.format( totalprice+3000 ) %></span>
			</p>
		</div>
		<div class="row">
