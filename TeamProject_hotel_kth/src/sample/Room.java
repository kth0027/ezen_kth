...
public class Room {
	...
	void roomPerson(int room, int number, String name) { // 예약자를 할당하는 메소드
		switch (room) {
		case 1:
			s[number][0] = name;
			s[number][1] = "1";
	...
	String getName() { // 룸 예약자 이름을 받는 메소드
		System.out.print("이름>>");
	...
	int getRoomNumber() { // 룸 번호를 받는 메소드
		System.out.print("번호>>");
		Scanner input = new Scanner(System.in);
	...
	int roomChk(int room, int number, String name) { // 이미 예약되어 있는지를 판별하는 메소드
		int ret = 0;
		...
		case 1:
			...
			{
				System.out.println("해당 룸이 예약되어있습니다. 다시 예약해 주세요.");
				ret = 0;
			}

			else // 예약에 성공 한 경우
			{
				roomPerson(room, number, name);
