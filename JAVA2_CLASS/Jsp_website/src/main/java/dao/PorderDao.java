...
public class PorderDao extends DB {
	...
	public ArrayList<Porder> getporderlist(int m_num) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				Porder porder = new Porder(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4),
	...
	public ArrayList<Porderdetail> getPorderdetaillist(int order_num) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				Porderdetail porderdetail = new Porderdetail(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4),
						rs.getInt(5));
	...
	// 날짜별 주문수
	public org.json.simple.JSONObject getorderdatecount() {
