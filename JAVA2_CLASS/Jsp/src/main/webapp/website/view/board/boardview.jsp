...
<body>
	...
	<div class="container">
		...
		<table class="table">
			...
			<tr>
				<td>첨부파일 다운로드 <br> <% if( board.getB_file() == null){ %> <% }else{%> <a href="../../controller/filedowncontroller.jsp?file=<%=board.getB_file()%>"><%=board.getB_file() %></a> <% }%>
				</td>
				<% if( board.getB_file() == null){ %>
				<td colspan="2" height="300px;"></td>
				<% }else{%>
				<td colspan="2" height="300px;">
					<!-- 박스권 안에 사진 사이징 : max-width , max-height --> 미리보기<br> <img src="../../upload/<%=board.getB_file()%>" style="max-width: 100%; max-height: 100%">
				</td>
				<% }%>
			</tr>
		</table>
