...
public class ShoppingCart {
	...
	// loops through the shopping list and prints out each item with the quantity of
	// each other
	public static void printList() {
		System.out.println("your cart contains: ");
		for (Object i : shoppingList) {
			System.out.println(((Item) i).returnQuantity() + " " + ((Item) i).returnName() + "(s), Costing"
					+ ((Item) i).returnPrice() + "each");
		}
	...
	public static void search(int i) {
		...
		for (Object o : shoppingList) {
			if (Inventory.getItem(i).returnName().equals(((Item) o).returnName())) {
				System.out.println(
						"You have " + ((Item) o).returnQuantity() + " " + ((Item) o).returnName() + "(s) in your cart");
				count++;
			}
		}
		if (count == 0) {
