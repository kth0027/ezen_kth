...
// -2. 회권가입 메소드 [ 입력받은 정보로 객체 생성하여 배열에 저장 ]
	public static boolean signup() {
		...
		String m_아이디 = ShopApplication.scanner.next();
		// 아이디 중복체크
		...
		String m_비밀번호 = ShopApplication.scanner.next();
		System.out.println(" 이름 : ");
		String m_이름 = ShopApplication.scanner.next();
		System.out.println(" 생년월일(YY-MM-DD) : ");
		String m_생년월일 = ShopApplication.scanner.next();

		// --2. 입력받은 정보로 객체 생성 [ 생성자 ]
		Member member = new Member(m_아이디, m_비밀번호, m_이름, m_생년월일);

		// 3. 만든 객체를 배열에 넣기 [ 빈공간을 찾아서 넣기 ]
		for (int i = 0; i < ShopApplication.members.length; i++) {
			if (ShopApplication.members[i] == null) {
				ShopApplication.members[i] = member;
...
// -6. 아이디 중복 체크 메소드
	public static boolean idcheck(String check_id) {
		for (int i = 0; i < ShopApplication.members.length; i++) {
			if (ShopApplication.members[i] != null && ShopApplication.members[i].getM_아이디().equals(check_id)) {
...
// -3. 로그인 메소드
	public static String login() {
		...
		for (int i = 0; i < ShopApplication.members.length; i++) {
			if (ShopApplication.members[i] != null 
					&& ShopApplication.members[i].getM_아이디().equals(m_아이디)
					&& ShopApplication.members[i].getM_비밀번호().equals(m_비밀번호)) {
...
// -4. 아이디찾기 메소드 ( 이름, 생년월일 입력받아 => 아이다 찾기 )
	public static String findid() {
		...
		for (int i = 0; i < ShopApplication.members.length; i++) {
			if (ShopApplication.members[i] != null 
					&& ShopApplication.members[i].getM_이름().equals(m_이름)
					&& ShopApplication.members[i].getM_생년월일().equals(m_생년월일)) {
				return ShopApplication.members[i].getM_아이디(); // 아이디 찾기 성공
			}
		}
	...
	public static String findpw() {
		...
		for (int i = 0; i < ShopApplication.members.length; i++) {
			if (ShopApplication.members[i] != null && 
					ShopApplication.members[i].getM_아이디().equals(m_아이디)
					&& ShopApplication.members[i].getM_생년월일().equals(m_생년월일)) {
				return ShopApplication.members[i].getM_비밀번호(); // 아이디 찾기 성공
			}
		}
